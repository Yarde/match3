...
namespace P2.Stats
{
    public class StatsSystem : IDisposable
    {
        ...
        public StatsSystem(ScoringSystem scoringSystem, ObjectivesSystem objectivesSystem)
        {
            ...
            scoringSystem.Score.Subscribe(_ => _score.Value += scoringSystem.Score.Value - scoringSystem.Score.PreviousValue).AddTo(_disposable);
            _wins.Subscribe(_ => _winLossRatio.Value = _wins.Value / (float) (_wins.Value + _losses.Value)).AddTo(_disposable);
            _losses.Subscribe(_ => _winLossRatio.Value = _wins.Value / (float) (_wins.Value + _losses.Value)).AddTo(_disposable);
